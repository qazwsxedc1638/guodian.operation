...
public class WeixinLinksucaiController extends BaseController {
	...
	public void link(WeixinLinksucaiEntity weixinLinksucai,HttpServletRequest request, HttpServletResponse response) {
		...
		System.out.println("------------------weixinLinksucai----------test1-------------------backUrl:"+backUrl);
		//URL配置ID
		String id = request.getParameter("id");
		System.out.println("------------------weixinLinksucai----------test2-------------------id:"+id);
		//URL配置信息
		weixinLinksucai = systemService.getEntity(WeixinLinksucaiEntity.class, id);
		//微信公众账号ID
		String accountid = weixinLinksucai.getAccountid();
		System.out.println("------------------weixinLinksucai----------test3-------------------accountid:"+accountid);
		...
		System.out.println("------------------weixinLinksucai----------test4-------------------openid:"+openid);
		//跳转出链接
		String outer_link_deal = null;
		
	    String outUrl = weixinLinksucai.getOuterLink();
	    System.out.println("------------------weixinLinksucai----------test6-------------------outUrl:"+outUrl);
		...
		if(oConvertUtils.isEmpty(openid)){
			 outer_link_deal = remoteWeixinMethod.callWeixinAuthor2ReturnUrl(request, accountid, outUrl);
			 System.out.println("------------------weixinLinksucai----------test7-------------------outer_link_deal:"+outer_link_deal);
		}
		if(oConvertUtils.isEmpty(outer_link_deal)){
			...
			System.out.println("------------------weixinLinksucai----------test8-------------------outer_link_deal:"+outer_link_deal);
		}
		//-------------------------------------------------------------------------------------------------------------
		...
		try {
			...
			if(outer_link_deal.indexOf("https://open.weixin.qq.com")!=-1){
				...
				System.out.println("------------------weixinLinksucai----------test9-------------------outer_link_deal+sign:"+outer_link_deal+"&sign="+sign);
	...
	/**
	 * 链接跳转 -- Backup for base code but which is wrong
	 */
	@RequestMapping(params = "linkBackup")
	public void linkBackup(WeixinLinksucaiEntity weixinLinksucai,HttpServletRequest request, HttpServletResponse response) {
		//获取请求路径
		String backUrl = this.getRequestUrlWithParams(request);
		System.out.println("------------------weixinLinksucai----------test1-------------------backUrl:"+backUrl);
		//URL配置ID
	    ...
	    System.out.println("------------------weixinLinksucai----------test5-------------------requestQueryString:"+requestQueryString);
	    String outUrl = weixinLinksucai.getOuterLink();
	    System.out.println("------------------weixinLinksucai----------test6-------------------outUrl:"+outUrl);
	    if(oConvertUtils.isNotEmpty(requestQueryString)){
	    	outUrl = outUrl + requestQueryString;
	    }
	    System.out.println("------------------weixinLinksucai----------test7-------------------outUrl:"+outUrl);
	    //update-start--Author:scott  Date:20150809 for：【判断访问地址是否有附加参数，有的话原样带回去】----------------------
